class CreditsController < ApplicationController
  ...
  private
    ...
    def set_credit
      @credit = Credit.includes(:debtor).includes(:payments).find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def credit_params
      params.require(:credit).permit(:debtor_id, :date, :amount, :rate, :arrears_rate, :payment_period_in_months)
    end
end
